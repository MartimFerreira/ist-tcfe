...
\begin{table}[hbt!]
  ...
  \end{tabular}
  \caption{Extra results obtained from solving the equation systems due to us explicitly showing equations (5) to (8). These will be useful during the method comparison.}
  \label{tab:mesh}
\end{table}
...
In order to confirm that both methods lead to the same results, all nodal potentials and currents through resistors (as well as $V_b$, $V_c$, $I_b$ and $I_c$) were calculated using both methods. Some of these results come directly from solving the previously mentioned equation systems while others result from the application of Ohm's law to the resistors or of KCL to nodes connected to just 2 branches. The values obtained can be seen below.
...
\par
As can be seen above, both methods lead to the same results, proving their equivalence.
