...
The transformer was replaced by its ideal model, namely by a dependent voltage source between nodes $In1$ and $In2$ (that we named $V_2$) of value $v_S/n$,  and one dependent current source between nodes Begin1 and the left-side ground of value $I_2/n$, where $I_2$ is the current going from $In2$ to $In1$. In order to properly simulate this in \textit{ngspice}, an imaginary voltage source of 0$\;V$ was added in between $In2$ and the dependent voltage source.
